...
```
*(Также для обратной совместимости поддерживается старая схема `netrkn://config?...`)*
...
Файлы конфигурации должны иметь расширение **`.qwdtt`** или **`.netrkn`** и содержать внутри себя:
* Либо вышеуказанную **URI-ссылку** (`qwdtt://config?...` / `netrkn://config?...`).
