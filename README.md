...
- Auto‑restart: автоматический перезапуск `libclient.so` при падении с экспоненциальным бэкоффом.
- QoS/limits: опции в UI для лимитов трафика и приоритезации трафика через туннель.
- Profiles: сохранённые профили подключений (peer/хеши/пароль/потоки).
